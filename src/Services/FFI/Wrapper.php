...
namespace Core\Services\FFI {
    ...
    class Wrapper
    {
        ...
        public const RECORDS_TO_REMOVE = [
            '_IO_FILE_plus',
        ];

        public const VARS_TO_REMOVE = [
            '_IO_2_1_stdin_',
            '_IO_2_1_stdout_',
            '_IO_2_1_stderr_',
        ...
        protected function filterDeclarations(array $declarations): array
        {
            ...
            foreach ( $declarations as $declaration ) {
                ...
                } else if ( $declaration instanceof Decl\NamedDecl\TypeDecl\TagDecl\RecordDecl ) {
                    if ( ! in_array($declaration->name, self::RECORDS_TO_REMOVE) ) {
                        $result[] = $declaration;
                    }
                } else if ( $declaration instanceof Decl\NamedDecl\ValueDecl\DeclaratorDecl\FunctionDecl ) {
                ...
                } else if ( $declaration instanceof Decl\NamedDecl\TypeDecl\TagDecl\EnumDecl ) {
                    $result[] = $declaration;
                } else {
