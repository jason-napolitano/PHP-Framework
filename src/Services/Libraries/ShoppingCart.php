...
namespace Core\Services\Libraries {
    ...
    /**
     * ----------------------------------------------------------------------------
     * The ShoppingCart class is a basic port of the CodeIgniter 3 cart module for
     * CodeIgniter 4.
     * ----------------------------------------------------------------------------
     *
    ...
    class ShoppingCart implements ShoppingCartInterface
    {
         ...
         * product ID and product name alpha-numeric, dashes, underscores, or
         * periods
         *
         * @var string
         */
         ...
         * ID and product name alpha-numeric, dashes, underscores, colons or periods
         *
         * @var string
         */
         ...
         *
         * The constructor loads the Session class, used to store the shopping
         * cart contents.
         */
        public function __construct()
        {
            ...
            if ( $this->cartContents === null || $this->cartContents === [] ) {
        ...
        public function insert(array $items = []): bool
        {
            ...
            if ( ! is_array($items) || count($items) === 0 ) {
                // log_message('error', 'The insert method must be passed an array
                // containing data.');
                return false;
            }

            // You can either insert a single product using a one-dimensional
            // array,
            // or multiple products using a multi-dimensional one. The way we
            // determine the array type is by looking for a required array key
            // named "id"
            // at the top level. If it's not found, we will assume it's a multi
            //dimensional array.
         ...
         * @return bool|string
         */
        protected function _insert($items = []): bool|string
        {
            // Was any cart data passed? No? Bah...
            if ( ! is_array($items) || count($items) === 0 ) {
                // log_message('error', 'The insert method must be passed an array
            ...
            if ( ! isset($items['id'], $items['qty'], $items['price'], $items['name']) ) {
                // log_message('error', 'The cart array must contain a product ID,
                // quantity, price, and name.');
                return false;
            }

            // --------------------------------------------------------------------

            // Prep the quantity. It can only be a number.  Duh... also trim any
            // leading zeros
            ...
            // Not totally sure we should impose this rule, but it seems prudent t
            //o standardize IDs.
            // Note: These can be user-specified by setting the $this->product_id_rules
            // variable.
            if ( ! preg_match('/^[' . $this->productIdRules . ']+$/i', $items['id']) ) {
                // log_message('error', 'Invalid product ID.  The product ID can only
                // contain alpha-numeric characters, dashes, and underscores');
                return false;
            }

            // --------------------------------------------------------------------

            // Validate the product name. It can only be alpha-numeric, dashes, underscores,
            // colons or periods.
            // Note: These can be user-specified by setting the $this->product_name_rules
            // variable.
            ...
            // We now need to create a unique identifier for the item being inserted into the
            // cart.
            // Every time something is added to the cart it is stored in the master cart array.
            // Each row in the cart array, however, must have a unique index that identifies not
            // only
            // a particular product, but makes it possible to store identical products with different
            // options.
            // For example, what if someone buys two identical t-shirts (same product ID), but in
            // different sizes?  The product ID (and other attributes, like the name) will be identical
            // for
            // both sizes because it's the same shirt. The only difference will be the size.
            // Internally, we need to treat identical submissions, but with different options, as a
            // unique product.
            // Our solution is to convert the options array to a string and MD5 it along with the
            // product ID.
            ...
            // Re-create the entry, just to make sure our index contains only the data from this
            // submission
            $items['rowid'] = $rowid;
         ...
         * Has options
         *
         * Returns TRUE if the rowid passed to this function correlates to an
         * item
         ...
         * @return bool
         */
        public function hasOptions(string $row_id = ''): bool
        {
         ...
         * @return mixed
         */
        public function productOptions(string $row_id = ''): mixed
        {
         ...
         * @return string
         */
        public function formatNumber(string $n = ''): string
        {
